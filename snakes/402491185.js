...
function Snake(setval){//蛇对象
	...
	this.move=function(){
		...
		this.headIndex=this.inde(this.body[0][0],this.body[0][1]);
		if(val!=this.foodIndex&&val>=0&&val<this.divs.length){this.divs[val].style.backgroundColor='transparent';}//将蛇尾设置为透明
		...
		for(var i=1;i<this.body.length;i++){
			...
			if(a0==this.headIndex){
				...
				this.continue=this.input[0].checked;
				if(!this.continue){this.restart();return;}else{
				if(setHead(this.headIndex+1)){console.log(this.headIndex+1);}else
				if(setHead(this.headIndex-1)){console.log(this.headIndex-1);}else
				if(setHead(this.headIndex+40)){console.log(this.headIndex+40);}else
				if(setHead(this.headIndex-40)){console.log(this.headIndex-40);}}
			}
		}
		function setHead(num){//改变蛇头程序
			var isSome=true;
			if(num>=0&&num<=1199){
		...
		if(!this.last){//是否不存在撞墙或者咬到自己
			for(var i=length-1;i>0;i--){//蛇身体的运动在这里产生
